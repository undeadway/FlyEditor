...
(function() {
	...
	var FACE_NAME = [ '黑线', '怒', '眼泪', '炸毛', '蛋定', '微笑', '汗', '囧', '卧槽', '坏笑', '鼻血', '大姨妈', '瞪眼', '你说啥', '一脸血', '害羞',
			'大好', '喝茶看戏', '美～', '笑岔', '中箭', '呕', '撇嘴', '碎掉', '吐舌头', '纳尼', '泪流满面', '升仙', '扭曲', '闪闪亮', '山', '寨', '基', '惊',
			'头顶青天', '不错', '吃屎', '牛', '严肃', '作死', '帅'/*, '僵尸', '吸血鬼', '喵'*/, '腹黑', '喜闻乐见', '呵呵呵', '！', '？', '吓尿了', '嘁',
			'闪电', "S1", "战斗力爆表", "贼笑", "嗯...", "喵" ];
	...
	var that = {
		toHTML : function(str, flg) {
			...
			str = str.replace(/\[(\/)?(sub|sup|strike|p|pre|i|b|u|tr|td)]/g, '<$1$2>');
			...
			str = str.replace(/\[align=(left|center|right|justify)\]([^\[]+)\[\/align\]/g, '<div align="$1">$2</div>');
			str = str.replace(/\[bgcolor=\#([^\[\<]+?)]/g, '<span class="bgcolor_$1">');
			str = str.replace(/\<(\/)?(tr|table)\>\<br \/\>/g, "<$1$2>");
			...
			str = str.replace(/\[list=(a|A|1)\](.+?)\[\/list]/g, '<ol type="$1">$2</ol>');
			str = str.replace(/\[list=(a|A|1)\]\[\/list]/g, '<ol type="$1"></ol>');
			str = str.replace(/\[(\/)?list\]/g, '<$1ul>');
			// styles.Image 模块的定义直接交给前台自定义实现，FlyEditor 相当于只给一个接口而不具体负责实现
			str = str
					.replace(/\[img\]([^\[]*)\[\/img\]/g, //[img]src[img]
					'<img class=\"flyimg\" title="$1" src="$1" onload="styles.Image.resize(this)" onclick="styles.Image.protoSize(this)" />');
			str = str
					.replace(
							/\[img align=(left|center|right|justify)\]([^\[]*)\[\/img]/g, //[img align=value]src[/img]
							'<img class=\"flyimg\" align="$1" alt="$2" title="$2" src="$2" onload="styles.Image.resize(this)" onclick="styles.Image.protoSize(this)" />');
			str = str
					.replace(
							/\[img=([^\[]*) align=(left|center|right|justify)\](.+?)\[\/img]/g, //[img=src align=value]title[img]
							'<img class=\"flyimg\" align="$2" src="$1" alt="$3" title="$3" onload="styles.Image.resize(this)" onclick="styles.Image.protoSize(this)" />');
			str = str
					.replace(
							/\[img=([^\[]*)](.+?)\[\/img\]/g, //[img=src]title[img]
							'<img class=\"flyimg\" src="$1" alt="$2" title="$2" onload="styles.Image.resize(this)" onclick="styles.Image.protoSize(this)" />');
			str = str.replace(/\[url](.+?)\[\/url\]/g, '<a href="$1">$1</a>'); // [url]link[url]
			str = str.replace(/\[url target=(.+?)\](.+?)\[\/url\]/g, '<a href="$2" title="$2" target="_$1">$2</a>'); // [url target=value]link[/url]
			str = str.replace(/\[url=([^\[]*) target=(.+?)\](.+?)\[\/url]/g,// [url=link target=value]title[/url]
			'<a href="$1" title="$3" target="_$2">$3</a>');
			str = str.replace(/\[url=([^\[]*)\](.+?)\[\/url\]/g, '<a href="$1" title="$2">$2</a>'); // [url=link]title[url]
			str = str.replace(/\[quote\]/g, '<fieldset class="fieldset"><legend class="legend">引用</legend><div>');
			str = str.replace(/\[quote=([^\]]+)\]/g, '<fieldset class="fieldset"><legend class="legend">$1</legend><div>');
			...
			str = str.replace(/\[underline\](.+?)\[\/underline]/g, '<u>$1</u>');

			return str;
		},
		clear : function(str) {
			str = str.replace(/\[\/(size|color|font|backcolor)\]/g, EMPTY_STRING);
			str = str.replace(/\[(\/)?(sub|flash|sup|underline|i|p|strike|b|quote|tucao|magnet|ed2k|thunder)\]/g,
					EMPTY_STRING);
